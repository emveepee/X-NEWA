...
class XNEWA_Connect:
    ...
    # Instantiation
    def __init__(self, ip, port, offline=False):
        self.ip = ip
        self.port = port
        self.vlc_url = None
        ...
        print self.mycache
        if not os.path.exists(xbmc.translatePath('special://temp') + 'x-newa/covers/'):
            os.makedirs(xbmc.translatePath('special://temp') + 'x-newa/covers/')
        
        self.myiconcache = xbmc.translatePath('special://temp') + 'x-newa/covers/'
        self.fanart = fanart()
...
######################################################################################################
    ...
    def checkCache(self,cached):
        if os.path.isfile(self.mycache + cached) == True:
            if self.offline:
                return True
            ft =  datetime.datetime.fromtimestamp(os.path.getmtime(self.mycache + cached))
    ...
    def cleanCoverCache(self):
        import glob
        print "clean covers"
        fileNames = glob.glob(self.myiconcache+'*.jpg')
        for file in fileNames:
            os.remove(file)
        return True
    ...
    def myCachedPickle(self,pObject,cached):
        pickle.dump(pObject, open(self.mycache+cached,'wb'),pickle.HIGHEST_PROTOCOL)

    def myCachedPickleLoad(self,cached):
        return pickle.load(open(self.mycache+cached,'rb'))
...
######################################################################################################
    def getGuideInfo(self, userid, password, timeStart, timeEnd, group):
        ...
        cached = 'guideListing-' + str(timeStart).replace(':','')  + '.p'
        print cached
        if self.checkCache(cached):
...
######################################################################################################
    def _detail2array(self, soapObj, fetchArt='No'):
        ...
        if fetchArt == 'Yes' and self.offline == False and self.fanart.getCachedIcon(dict['title']) is None:
            FanArt = None
            try:
                if prog is not None:
                    FanArt = prog.FanArt               
            ...
            if FanArt is not None:
                ...
                if mys==200:
                    try:
                        urllib.urlretrieve (url,self.myiconcache+dict['title']+'.jpg')
                    except StandardError:
